...
def _extract_links(msg: Message) -> List[MailLink]:
    """Extract body links before HTML is converted to plain text."""
    html_parts: List[str] = []
    text_parts: List[str] = []
    ...
    for html in html_parts:
        ...
        if HAS_BS4:
            ...
            for a in soup.find_all("a", href=True):
                ...
                parent_text = ""
                if a.parent:
                    parent_text = _clean_link_text(a.parent.get_text(" ", strip=True))
                key = (href, text, parent_text)
                if key in seen:
                    continue
                seen.add(key)
                out.append(MailLink(url=href, text=text, context=parent_text))
        else:
    ...
    for text in text_parts:
        for match in _URL_RE.finditer(text or ""):
            ...
            context = _clean_link_text(text[start:end])
            key = (url, "", context)
            if key in seen:
                continue
            seen.add(key)
            out.append(MailLink(url=url, context=context))
